...
Explicitly not in scope for *that* stage: a per-room client UI, real credit charging, and durable job recovery. The first two have since been built (below); durable job recovery across restarts is still outstanding — an interrupted job is marked `interrupted`, never resumed.

**Follow-up commit (`864d4a0`, same branch, same day):** per owner direction, generation is now wired to live Gemini (no OpenAI, no credit charging yet):

- `backend/service/thecommons_generate.py` calls the suite's own `google_api.gen_text()` (Interactions/legacy dispatch, JSON-mode retry, safety-block translation) instead of a raw HTTP fetch, via `asyncio.to_thread` like every other suite router. One validation-aware repair pass on invalid JSON, flattened into a single user turn (original prompt → the model's own broken output → the validation error) rather than true multi-turn history — the suite's chat-turn primitive (`gen_chat`) doesn't support `system_instruction`/JSON-mode, and extending shared plumbing for one caller's fairly specific combination wasn't worth it. Network/HTTP/timeout failures skip the repair pass and fall back immediately, exactly matching `generate.js`. Uses `config.MODEL_TEMPLATE_GEN` (`gemini-3.1-pro-preview`) — the suite's own model for structurally similar JSON+code generation tasks, and the same model TheCommons' own live test used earlier.
- `/api/thecommons/generate` was already in `enforcement.AI_PREFIXES` from stage 2, so terms/rate-limit/the daily USD budget breaker already gate it — but nothing reserves or debits credits per call. That's still the deferred piece.
- `GET /api/thecommons/qr/{join_code}`: a per-room join QR as a PNG, via the `qrcode` PyPI package (new `requirements.txt` line; Pillow, its image backend, was already present). Public and unauthenticated, same trust model as `/ws/thecommons/{join_code}` — this is what a room's public display page shows for people to scan, so gating it behind the owner's session would defeat the point. **No QR-encoding logic was written**, on either side: this repo's own `scripts/generate-join-qr.js` was likewise always a thin wrapper around the `qrcode` npm package (`jsqr`/`pngjs` there are only for a one-time decode self-check on the static asset it writes, not encoding), and it can't run inside the FastAPI process anyway — no Node runtime there, per the serving-model decision. Encodes `{public origin}/thecommons/join/{join_code}` — that join page doesn't exist yet (no client UI this stage), same as the room CRUD endpoints existing before any creator-desk UI does.
**Credit metering (commit `5f369db`, same branch):** the spend-control gap that was blocking any deploy is now closed.

- New `commons_sketch` action in `pricing.py` at **10 credits** (2× the Pro text rate). One generation can legitimately be two Pro calls — the initial one plus the repair pass — and the repair is decided deep inside the generator, long after the request that could have surfaced a 402, so the worst case is reserved up front. That can never under-charge and needs no partial-refund mechanism (`Charge` has no notion of one). At 300 free credits/month that's 30 generations per account per month.
- `jobs.start()` reserves **last**, after the idempotency check, the busy-room check and the remix-staleness check have all passed — so a replay, a busy room, or a stale remix can never silently burn credits, and there's no refund-on-rejection dance. Still inside the room lock, so reserve and insert can't interleave. The reservation's `generations.id` lands in the job row's `generation_id` column — the plug point stage 2 had already left for exactly this.
- **Refund policy**: the charge stands whenever the model actually answered (that response was billed by Google whether or not we could parse it — and "model replied with junk" is the one outcome a user can provoke deliberately, so refunding it would fund unlimited retries on the operator's key). Everything else refunds in full: no key configured, rejected input, transport failure. Admins stay exempt from the debit, as everywhere else in the suite.
- 9 new metering tests cover: reserve-before-dispatch, charge-stands-on-answer, charge-stands-on-unusable-output, refund-on-no-dispatch, refund-on-transport-failure, 402-with-no-job-created when short, rejected requests costing nothing, one owner's spend not moving another's balance, and admin exemption.

**Client UI (commit `b265534`, same branch).** The browser client is now ported into `static/thecommons/` and made multi-room. Unlike the server port, this is mostly the *original* code — it's plain browser JS with no Node dependency, and `display.js`/`station.js` had already solved the hard parts (dual-contract rendering, ownership-aware controls, drag coalescing at ~30/sec, reload-safe job following).

- `/thecommons/` — room list and creation, gated on the suite's existing Google session via `auth.js`'s `synth:auth-ready` event. **The `ADMIN_PASSWORD` login is gone entirely**; a verified Google session plus the server-side ownership check is the only thing that opens a desk.
- `/thecommons/desk/?room=<id>` — the creator desk. Keeps the original's request-ID job following, so a reload reattaches to a running job rather than firing a second paid call, now with per-room draft/job storage keys. Surfaces 402 out-of-credits distinctly.
- `/thecommons/display/{joinCode}` and `/thecommons/join/{joinCode}` — the wall and the participant station, room-scoped from the URL path, with per-room session/table keys. Two `FileResponse` routes back these because StaticFiles can't route a path segment and the join URL is a path (it's what the QR encodes).

**This was verified in a real browser against the full stack**, using a scratchpad dev server that runs the actual app against the test suite's in-memory pool (the machine has no Postgres). End to end: created a room → generated → the wall rendered the native Canvas2D piece → joined from a second tab → changed a palette control → **the wall repainted live** → loaded an inherited p5 template and confirmed the separate p5 path renders and labels itself "Library piece". Credit accounting through the real UI reserved 10 and refunded to 300 when no model answered. Station console clean. One real bug was found and fixed this way: `.panel { display: flex }` outranked the `hidden` attribute, so the signed-out panel showed while signed in.

**Migration verification status.** Neither Docker nor Postgres is installed on the dev machine, and no self-contained Postgres is available for Windows on PyPI, so the schema has still never run against a real server. What *was* done instead, statically: the whole of `schema.sql` (19 statements) and all 20 runtime SQL statements in the Commons modules parse cleanly against the **real PostgreSQL grammar** via `pglast`/libpg_query, and all 46 column references across every INSERT/UPDATE were cross-checked against the parsed schema. That matters because the test suite's fake pool matches SQL by substring, so a syntax error or a wrong column name would otherwise pass every test and only fail in production. Residual risk is low and fails safe: the migration is additive-only (`CREATE TABLE IF NOT EXISTS` + a version bump, nothing altered or dropped), it runs at app startup, and Cloud Run does not route traffic to a revision that fails to start — so a broken migration would leave the previous revision serving rather than breaking the live site.

- Testing this surfaced two real gaps, now fixed: the fake Postgres pool used by these tests was missing a `fetchval` handler for the budget breaker's query (silently logging a full traceback and failing open on every generate call instead of being exercised for real), and the end-to-end isolation test was driving live HTTP through the real generator **without mocking `ai_manager.genai_client`** — since this machine has a real Gemini key configured locally, that test was making actual billed API calls, taking 2+ minutes and violating this project's own "keep tests off paid providers" invariant. Both fixed; full suite is now 349 passing in ~14s.
...
### 4. Add room creation, display links, and per-room QR codes — largely done

Built in commits `864d4a0` (QR endpoint) and `b265534` (client UI). Room creation and ownership run on the suite's Google session; the creator's room list/desk and the room-scoped display/participant routes all exist; QR codes are generated per room from the canonical public origin (`SYNTH_PUBLIC_ORIGINS`, not the request Host — the Vercel proxy would otherwise bake the internal `run.app` URL into every code) and encode nothing but the join link.

Still open from this stage, and the reason it's "largely" rather than "done": **invitation lifecycle**. A room has a `status` column and unknown/closed rooms already close the socket with 4404 and 404 the QR, but there is no way to close, reopen, expire, or rotate a room's invitation yet, and no UI for it. The acceptance criterion involving *real phones* also remains untested — everything so far was verified in a desktop browser against a local dev harness.
