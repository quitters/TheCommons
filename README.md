...
- Fresh-generation checks (`server/validate-sketch.js`): native-only output, named controls with distinct weighted choices, matching prompt placeholders, and compilable JavaScript before broadcast. The request times out after 45 seconds into the same fallback pool. Syntax validation does not execute model code on the server or guarantee its visual quality; the display still handles per-frame runtime errors.
